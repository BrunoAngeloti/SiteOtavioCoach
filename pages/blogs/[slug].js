      ...
      <div className={styles.content}>
        ...
        {blog.link_para_conteudo_externo.url && (
          <span className={styles.externo}>
            {
              <a
                href={blog.link_para_conteudo_externo.url}
                target="_blank"
                rel="noreferrer"
              >
...
export const getServerSideProps = async (context) => {
  try {
    ...
    const client = await Prismic.client(
      "https://angeloticoach.prismic.io/api/v2"
    );
    const blog = await client.query(
      Prismic.Predicates.at("document.type", "blog")
    );

    const blogFilter = blog.results.filter(
      (blog) => formatarParaURL(blog.data.titulo[0].text) === slug
    );
